...
public class RobotContainer {
    ...
    private final SwerveInputStream driveAngularVelocity = SwerveInputStream.of(drivebase.getSwerveDrive(),
            ...
            .withControllerRotationAxis(() -> rotLimiter.calculate(-controller1.getRightX()))
    ...
    public RobotContainer() {
        ...
        autoChooser.addRoutine("2 Coral: Left 2",
                () -> autos.get2CoralDriveRoutine("Score Left 2 G", "Left 2 Intermediate Pickup", "Score Pickup I"));
        autoChooser.addRoutine("UNTESTED - 2 Coral: Right 2",
                () -> autos.get2CoralDriveRoutine("Score Right 2 D", "Right 2 Intermediate Pickup", "Score Pickup F"));
        autoChooser.addRoutine("3 Coral Left 2",
                () -> autos.get3CoralDriveRoutine("3 - Blue 2 G Pickup", "3 - I Score", "3 - I Pickup", "3 - J Score"));
        // Controller Chooser
        bindAlternative();
        bindAutomatic();
        bindManual();
       ...
       // bindTwisted();
        ...
        controllerModeChooser.setDefaultOption("TWISTED", fullAutomaticEventLoop);
    ...
    public void bindTwisted() {
        ...
        controller1.rightTrigger(0.5, twistedEventLoop)
                .onTrue(new InstantCommand( () -> new DropCoral(elevator, arm, wrist, selectedLevel,
                        wristLeft ? WristSetpoints.VERTICAL_L : WristSetpoints.VERTICAL_R)
                        .unless(() -> selectedLevel == 1).schedule()))
        ...
        controller1.leftTrigger(0.5, twistedEventLoop)
                ...
                .onTrue(new ConditionalCommand(
                        new MoveToolingToSetpoint(elevator, arm, wrist, ElevatorSetpoints.GROUND_CORAL,
                                ArmSetpoints.GROUND_CORAL, WristSetpoints.HORIZONTAL, true),
                        new MoveToolingToSetpoint(elevator, arm, wrist, ElevatorSetpoints.CORAL_STATION,
                                ArmSetpoints.CORAL_STATION, WristSetpoints.HORIZONTAL, true),
                        () -> groundIntake));

        // -- CONTROLLER 2 --
        controller2.leftTrigger(0.5, twistedEventLoop).onTrue(new InstantCommand(() -> {
            groundIntake = true;
        }));
        controller2.rightTrigger(0.5, twistedEventLoop).onTrue(new InstantCommand(() -> {
            groundIntake = false;
        }));

        controller2.leftBumper(twistedEventLoop).onTrue(new InstantCommand(() -> {
            wristLeft = true;
        }));
        controller2.rightBumper(twistedEventLoop).onTrue(new InstantCommand(() -> {
            wristLeft = false;
        }));
        ...
        controller2.a(twistedEventLoop).onTrue(new InstantCommand(() -> {
            new MoveToolingToSetpoint(elevator, arm, wrist, ElevatorSetpoints.CORAL_L1, ArmSetpoints.CORAL_L1,
                    WristSetpoints.HORIZONTAL, true);
            selectedLevel = 1;
        }));
    }
    ...
    public void bindAutomatic() {
        ...
        // You have to wrap the command options in another command because Java compiles
        // conditionals returning values at compile time, not runtime
        // Alternative option would be to decorate 4+ commands with the .onlyIf() and
        // .alongWith() decorators (not great)
        HashMap<LevelSelectorKey, Command> placingCommands = new HashMap<>();
        placingCommands.put(LevelSelectorKey.CORAL_L1, new MoveToolingToSetpoint(elevator, arm, wrist,
                ElevatorSetpoints.CORAL_L1, ArmSetpoints.CORAL_L1, WristSetpoints.HORIZONTAL, true));
        placingCommands.put(LevelSelectorKey.CORAL_L2_L,
                new PlaceCoral(elevator, arm, wrist, 2, WristSetpoints.VERTICAL_L));
        placingCommands.put(LevelSelectorKey.CORAL_L2_R,
                new PlaceCoral(elevator, arm, wrist, 2, WristSetpoints.VERTICAL_R));
        placingCommands.put(LevelSelectorKey.CORAL_L3_L,
                new PlaceCoral(elevator, arm, wrist, 3, WristSetpoints.VERTICAL_L));
        placingCommands.put(LevelSelectorKey.CORAL_L3_R,
                new PlaceCoral(elevator, arm, wrist, 3, WristSetpoints.VERTICAL_R));
        placingCommands.put(LevelSelectorKey.CORAL_L4_L,
                new PlaceCoral(elevator, arm, wrist, 4, WristSetpoints.VERTICAL_L));
        placingCommands.put(LevelSelectorKey.CORAL_L4_R,
                new PlaceCoral(elevator, arm, wrist, 4, WristSetpoints.VERTICAL_R));
        ...
        ConditionalCommandChooser<LevelSelectorKey> placeWrapper = new ConditionalCommandChooser<>(placingCommands,
                this::getLevelSelectorKey);
        ...
        scoringCommands.put(LevelSelectorKey.CORAL_L2_L,
                new DropCoral(elevator, arm, wrist, 2, WristSetpoints.VERTICAL_L));
        scoringCommands.put(LevelSelectorKey.CORAL_L2_R,
                new DropCoral(elevator, arm, wrist, 2, WristSetpoints.VERTICAL_R));
        scoringCommands.put(LevelSelectorKey.CORAL_L3_L,
                new DropCoral(elevator, arm, wrist, 3, WristSetpoints.VERTICAL_L));
        scoringCommands.put(LevelSelectorKey.CORAL_L3_R,
                new DropCoral(elevator, arm, wrist, 3, WristSetpoints.VERTICAL_R));
        scoringCommands.put(LevelSelectorKey.CORAL_L4_L,
                new DropCoral(elevator, arm, wrist, 4, WristSetpoints.VERTICAL_L));
        scoringCommands.put(LevelSelectorKey.CORAL_L4_R,
        ...
        ConditionalCommandChooser<LevelSelectorKey> scoreWrapper = new ConditionalCommandChooser<>(scoringCommands,
                this::getLevelSelectorKey);

        controller1.rightTrigger(0.5, fullAutomaticEventLoop).onTrue(scoreWrapper);
        controller1.leftBumper(fullAutomaticEventLoop).onTrue(new MoveToolingToSetpoint(elevator, arm, wrist,
                ElevatorSetpoints.STOW, ArmSetpoints.STOW, WristSetpoints.HORIZONTAL));

        controller1.a(fullAutomaticEventLoop).onTrue(
                new MoveToolingToSetpoint(elevator, arm, wrist, ElevatorSetpoints.GROUND_CORAL,
                        ArmSetpoints.GROUND_CORAL, WristSetpoints.HORIZONTAL));
        ...
        controller1.x(fullAutomaticEventLoop).onTrue(
                new MoveToolingToSetpoint(elevator, arm, wrist, ElevatorSetpoints.CORAL_STATION,
                        ArmSetpoints.CORAL_STATION, WristSetpoints.HORIZONTAL));

        // slow mode
        controller1.leftTrigger(0.5, fullAutomaticEventLoop).onTrue(drivebase.changeSwerveSpeed(0.2))
    ...
    public void bindCommonControls(EventLoop loop) {
        ...
        new Trigger(loop, DriverStation::isAutonomousEnabled)
                .whileTrue(new CustomWaitCommand(() -> autoWaitTime).andThen(autoChooser.selectedCommandScheduler()));
        new Trigger(loop, Utilities::isRedAlliance)
                .onChange(new InstantCommand(FieldLocation::recalculateFieldPositions).ignoringDisable(true));
        ...
        Dashboard.getNetworkTablesButton("Speed Down", loop).onTrue(drivebase.changeSwerveSpeed(0.1))
                .onFalse(drivebase.changeSwerveSpeed(0.5));
    }
