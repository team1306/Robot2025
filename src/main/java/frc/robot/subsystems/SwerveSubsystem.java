...
public class SwerveSubsystem extends SubsystemBase {
    ...
    public Command driveCommand(DoubleSupplier translationX, DoubleSupplier translationY, DoubleSupplier angularRotationX) {
        return run(() -> {
            ...
            final double forwardComponent = smartPow(translationX.getAsDouble(), 2) * swerveDrive.getMaximumChassisVelocity();
            final double sidewaysComponent = smartPow(translationY.getAsDouble(), 2) * swerveDrive.getMaximumChassisVelocity();
            final double angularComponent = smartPow(angularRotationX.getAsDouble(), 2) * swerveDrive.getMaximumChassisAngularVelocity();
    ...
    public Command getAutoCommand() {
        ...
        AutoFactory builder = new AutoFactory(this::getPose, this::resetOdometry, (sample) -> {
            PIDController xController = new PIDController(10, 0, 0);
            PIDController yController = new PIDController(10, 0, 0);
            PIDController rController = new PIDController(10, 0, 0);
            ...
            ChassisSpeeds speeds = ChassisSpeeds.fromFieldRelativeSpeeds(
                new ChassisSpeeds(
                    xController.calculate(getPose().getX(), desiredPose.getX()) + desiredSpeeds.vxMetersPerSecond,
                    yController.calculate(getPose().getY(), desiredPose.getY()) + desiredSpeeds.vyMetersPerSecond,
                    rController.calculate(getPose().getRotation().getRadians(), desiredPose.getRotation().getRadians()) + desiredSpeeds.omegaRadiansPerSecond
                ), getPose().getRotation());
            setChassisSpeeds(speeds);
            }, this.isRedAlliance(), this, new AutoFactory.AutoBindings());
           
        
        return new InstantCommand(() -> resetOdometry(((choreo.trajectory.Trajectory<?>) trajectory.get()).getInitialPose(isRedAlliance()).get()))
            .andThen(builder.trajectoryCmd((choreo.trajectory.Trajectory<?>) trajectory.get()));
    }
    ...
    public Command aimAtSetpoint(Rotation2d rotation, Rotation2d tolerance) {
        //TODO Likely NOT WORKING BECAUSE DRIVE RESETS BACK TO WHAT IT LAST WAS
    ...
    public double[] getWheelRadiusCharacterizationPositions() {
        ...
        for(int i = 0; i < 4; i++){
            pos[i] = ((TalonFX)(swerveDrive.getModules()[i].getDriveMotor().getMotor())).getRotorPosition().getValue();
        }
    ...
    public Rotation2d getGyroAngle(){
        return ((AHRS)(swerveDrive.getGyro().getIMU())).getRotation2d();
    }
}
