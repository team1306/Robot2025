...
public class FieldLocation {
    ...
    public static final Set<Integer> REEF_APRILTAG_IDS = new HashSet<>(Arrays
            .asList(22, 21, 19, 18, 17, 6, 7, 8, 9, 10, 11));
    ...
    public static void recalculateFieldPositions() {
        ...
        CORAL_STATION_RIGHT = CORAL_STATION_RIGHT.transformBy(new Transform2d(-(coralStationOffset
                .in(Meters) + robotWidth.in(Meters)), Inches.of(2).in(Meter), Rotation2d.kZero));
        CORAL_STATION_LEFT = CORAL_STATION_LEFT.transformBy(new Transform2d(-(coralStationOffset.in(Meters) + robotWidth
                .in(Meters)), 0, Rotation2d.kZero));
    ...
    public static Pose2d getReefOffsetFromAprilTag(ReefSide reefSide) {
        if (REEF_APRILTAG_IDS.contains((int) LimelightHelpers.getFiducialID(Constants.LIMELIGHT_4_NAME)))
            return Pose2d.kZero;
        Pose3d fiducialPos = LimelightHelpers.getTargetPose3d_RobotSpace(Constants.LIMELIGHT_4_NAME);

        return switch (reefSide) {
            case LEFT -> new Pose2d(fiducialPos.getMeasureX().minus(reefOffset), fiducialPos.getMeasureZ(), fiducialPos
                    .getRotation()
                    .toRotation2d());
            case RIGHT -> new Pose2d(fiducialPos.getMeasureX().plus(reefOffset), fiducialPos.getMeasureZ(), fiducialPos
                    .getRotation()
                    .toRotation2d());
        };
    }
    ...
    private static Pose2d flipToRedSide(Pose2d pose) {
        if (FlippingUtil.symmetryType != FlippingUtil.FieldSymmetry.kMirrored) System.out
                .println("Symmetry type changed to mirrored");
