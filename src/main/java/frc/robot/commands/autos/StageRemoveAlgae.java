...
public class StageRemoveAlgae extends SequentialCommandGroup {
    public StageRemoveAlgae(Elevator elevator, Arm arm, Wrist wrist, int level) {
        if (level != 2 && level != 3) throw new RuntimeException("Level must be 2 or 3");

        ElevatorSetpoint elevatorSetpoint = level == 2 ? ElevatorSetpoints.ALGAE_L2_REMOVE : ElevatorSetpoints.ALGAE_L3_REMOVE;
